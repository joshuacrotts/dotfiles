...
builtin_functions_init(struct environment *env) {
  ...
  environment_put(env, "char->string", sval_builtin_create(apply_char_to_string));
...
static struct sval *
apply_char_to_string(struct sval **args, size_t num_args, struct environment *env) {
  ASSERT_ARITY("char->string", 1, num_args);
  struct sval *character = args[0];
  char ch = character->data.character;
  char *s = mgc_alloc(2, free);
  s[0] = ch;
  return sval_string_create(s);
}
