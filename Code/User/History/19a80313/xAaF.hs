...
elemIndex' e [] = Nothing
elemIndex' e (x : xs) = elemIndexHelper 0 xs
    where
        elemIndexHelper :: Eq a => Int -> [a] -> Maybe Int
        elemIndexHelper _ [] = Nothing
        elemIndexHelper i (x : xs) | e == x = Just i
                                   | otherwise = elemIndexHelper (i + 1) xs