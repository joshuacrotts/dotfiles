...
Of course, this never terminates.\footnote{It does eventually crash because we are liberal with how we free dynamic memory\index{dynamic memory}.} In the old implementation, this would quickly result in a segmentation fault\index{segmentation fault}. Trying a very large input on the tail recursive\index{tail recursive} factorial function also no longer immediately segfaults.
