...
-- (A => B) => (B => C) => (A => C)
comp1 :: (a -> b) -> (b -> c) -> (a -> c)
comp1 fab fbc = fbc . fab


-- (B => C) => (A => B) => (A => C)
comp2 :: (b -> c) -> (a -> b) -> (a -> c)
comp2 fbc fab = fbc . fab


-- (A => B => C) => (A => B) => A => C
comp3 :: (a -> b -> c) -> (a -> b) -> a -> c
comp3 fabc fab a = (fabc a) (fab a)
...
-- -- (A ∨ (B ∧ C)) => (A ∨ B) ∧ (A ∨ C)
-- distrOr :: (Either a (b, c)) -> ((Either a c), (Either b c))
-- distrOr eitherABC = case eitherABC of
